...
#error "Please define a way to initialize a memory block."
	...
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %f\n",default_num_contexts*results[0].iterations/time_in_secs((ee_u32)total_time));
#else 
	ee_printf("Total time (secs): %d\n",time_in_secs((ee_u32)total_time));
	if (time_in_secs((ee_u32)total_time) > 0)
		ee_printf("Iterations/Sec   : %d\n",default_num_contexts*results[0].iterations/time_in_secs((ee_u32)total_time));
#endif
	if (time_in_secs((ee_u32)total_time) < 10) {
