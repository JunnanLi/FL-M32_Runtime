...
void sys_gettime(struct time_spec *timer){
  uint32_t *addr = (uint32_t *) TIMER_NS_ADDR;
  timer->tv_nsec = *((volatile uint32_t*)addr);
  timer->tv_sec = *((volatile uint32_t*)addr+1);
}
...
long time(long * value)
{
	int cycles;
	int addr = 0xB00;
	__asm__ __volatile__ ("csrr %0, %1" : "=r"(cycles)
					:"i"(0xB00));
...
long insn(long * value)
{
	int insns;
	int addr = 0xB02;
	__asm__ __volatile__ ("csrr %0, %1" : "=r"(insns)
					:"i"(0xB00));
	// printf("[insn() -> %d]", insns);
	return insns;
}
